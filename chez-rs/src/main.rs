...
fn main() {
    ...
    if let Some(ref boot) = args.boot {
        for i in boot {
            register_boot_executable_relative_file(exec_path, i.to_str().unwrap());
        }
    }
    ...
    if args.quiet {
        call1("suppress-greeting", boolean(true));
    ...
    if args.enable_object_counts {
        call1("enable-object-counts", boolean(true));
    }
    ...
    if args.debug_on_exception {
        call1("debug-on-exception", boolean(true));
    }

    if args.import_notify {
        call1("import-notify", boolean(true));
    }
    ...
    if args.compile_imported_libraries {
        call1("compile-imported-libraries", boolean(true));
    }
    ...
    let trailing = std::env::args()
        .take(1)
        .chain(args.trailing)
        .map(|i| std::ffi::CString::new(i.clone()).expect("CString::new failed"))
        .collect::<Vec<_>>();
